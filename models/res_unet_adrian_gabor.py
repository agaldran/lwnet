...
def conv1x1(in_planes, out_planes, stride=1):
    return GaborConv2d(in_planes, out_planes, kernel_size=1, stride=stride, bias=False)
...
class ConvBlock(torch.nn.Module):
    def __init__(self, in_c, out_c, k_sz=3, shortcut=False, pool=True):
        ...
        block.append(GaborConv2d(in_c, out_c, kernel_size=k_sz, padding=pad))
        # block.append(nn.ReLU())
        block.append(nn.LeakyReLU())
        block.append(nn.BatchNorm2d(out_c))

        block.append(GaborConv2d(out_c, out_c, kernel_size=k_sz, padding=pad))
        # block.append(nn.ReLU())
        block.append(nn.LeakyReLU())
        block.append(nn.BatchNorm2d(out_c))
...
class UpsampleBlock(torch.nn.Module):
    def __init__(self, in_c, out_c, up_mode='transp_conv'):
        ...
        if up_mode == 'transp_conv':
            ...
            block.append(GaborConv2d(in_c, out_c, kernel_size=1))
        else:
...
class ConvBridgeBlock(torch.nn.Module):
    def __init__(self, channels, k_sz=3):
        ...
        block.append(GaborConv2d(channels, channels, kernel_size=k_sz, padding=pad))
        # block.append(nn.ReLU())
        block.append(nn.LeakyReLU())
...
class UNet(nn.Module):
    def __init__(self, in_c, n_classes, layers, k_sz=3, up_mode='transp_conv', conv_bridge=True, shortcut=True):
        ...
        for m in self.modules():
            if isinstance(m, (nn.Conv2d, GaborConv2d, nn.ConvTranspose2d)):
