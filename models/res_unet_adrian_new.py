...
class ConvBlock(torch.nn.Module):
    def __init__(self, in_c, out_c, k_sz=3, shortcut=False, pool=True, norm='bn'):
        '''
        ...
        if shortcut==True:
            if norm == 'bn':
                ...
                self.shortcut = nn.Sequential(conv1x1(in_c, out_c), nn.InstanceNorm2d(out_c, affine=True))
        else:
            self.shortcut=False
        pad = (k_sz - 1) // 2
        ...
        # block.append(nn.ReLU()) SELU(), LeakyReLU()
        block.append(nn.LeakyReLU())
        if norm=='bn':
            block.append(nn.BatchNorm2d(out_c))
        elif norm=='in':
            block.append(nn.InstanceNorm2d(out_c, affine=True))
...
class UpConvBlock(torch.nn.Module):
    def __init__(self, in_c, out_c, k_sz=3, up_mode='up_conv', conv_bridge=False, shortcut=False, norm='bn'):
        super(UpConvBlock, self).__init__()
        self.conv_bridge = conv_bridge

        self.up_layer = UpsampleBlock(in_c, out_c, up_mode=up_mode)
        self.conv_layer = ConvBlock(2 * out_c, out_c, k_sz=k_sz, shortcut=shortcut, pool=False, norm=norm)
        if self.conv_bridge:
            self.conv_bridge_layer = ConvBridgeBlock(out_c, k_sz=k_sz, norm=norm)
...
class UNet(nn.Module):
    def __init__(self, in_c, n_classes, layers, k_sz=3, up_mode='transp_conv', conv_bridge=True, shortcut=True, in_norm=False, norm='bn'):
        super(UNet, self).__init__()
        self.n_classes = n_classes
        self.in_norm=in_norm
        if in_norm:
            self.instance_norm = nn.InstanceNorm2d(num_features=in_c, affine=True)
            # self.instance_norm = nn.BatchNorm2d(num_features=in_c)

        self.first = ConvBlock(in_c=in_c, out_c=layers[0], k_sz=k_sz,
                               shortcut=shortcut, pool=False, norm=norm)

        self.down_path = nn.ModuleList()
        for i in range(len(layers) - 1):
            block = ConvBlock(in_c=layers[i], out_c=layers[i + 1], k_sz=k_sz,
                              shortcut=shortcut, pool=True, norm=norm)
        ...
        for i in range(len(layers) - 1):
            block = UpConvBlock(in_c=reversed_layers[i], out_c=reversed_layers[i + 1], k_sz=k_sz,
                                up_mode=up_mode, conv_bridge=conv_bridge, shortcut=shortcut, norm=norm)
        ...
        # # # forgot to add nn.ConvTranspose2d!!
        for m in self.modules():
            # if isinstance(m, nn.Conv2d):
