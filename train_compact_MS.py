...
def run_one_epoch(loader, model, criterion, optimizer=None, scheduler=None,
    ...
    for i_batch, (inputs, labels) in enumerate(loader):
        ...
        if isinstance(logits, tuple): # wnet
            ...
            loss += 0.5*criterion(torch.cat([-10 * torch.ones(labels.shape).to(device), logits_aux], dim=1), labels.squeeze(dim=1))
            loss += 0.5*criterion(torch.cat([-10 * torch.ones(labels.shape).to(device), logits], dim=1), labels.squeeze(dim=1))

            logits = torch.nn.functional.interpolate(logits, scale_factor=1/2, recompute_scale_factor=False)
            logits_aux = torch.nn.functional.interpolate(logits_aux, scale_factor=1/2, recompute_scale_factor=False)
            labels = torch.nn.functional.interpolate(labels.float(), scale_factor=1/2, recompute_scale_factor=False).long()
            loss += 0.25*criterion(torch.cat([-10 * torch.ones(labels.shape).to(device), logits_aux], dim=1), labels.squeeze(dim=1))
            loss += 0.25*criterion(torch.cat([-10 * torch.ones(labels.shape).to(device), logits], dim=1), labels.squeeze(dim=1))
