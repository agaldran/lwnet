...
def run_one_epoch(loader, model, criterion, tv_criterion, optimizer=None, scheduler=None,
    ...
    for i_batch, (inputs, labels) in enumerate(loader):
        ...
        if isinstance(logits, tuple): # wnet
            ...
            logits = torch.nn.UpsamplingNearest2d(scale_factor=1/2, recompute_scale_factor=False)(logits)
            logits_aux = torch.nn.UpsamplingNearest2d(scale_factor=1/2, recompute_scale_factor=False)(logits_aux)
            labels = torch.nn.UpsamplingNearest2d(scale_factor=1/2, recompute_scale_factor=False)(labels.float()).long()
