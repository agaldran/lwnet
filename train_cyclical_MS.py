        ...
        for i_batch, (inputs, labels) in enumerate(loader):
            ...
            if isinstance(logits, tuple): # wnet
                ...
                if model.n_classes == 1: # BCEWithLogitsLoss()/DiceLoss()
                    ...
                    logits = torch.nn.functional.interpolate(logits, scale_factor=1/2, recompute_scale_factor=False, mode='bicubic')
                    logits_aux = torch.nn.functional.interpolate(logits_aux, scale_factor=1/2, recompute_scale_factor=False, mode='bicubic')
                    labels = torch.nn.functional.interpolate(labels.float(), scale_factor=1/2,recompute_scale_factor=False).long()
                    loss += criterion(logits_aux, labels.float())
