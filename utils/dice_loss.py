...
class SimilarityLoss(torch.nn.Module):
    ...
    def forward(self, logits, labels, **kwargs):
        ...
        sim_along_veins = torch.nn.CosineSimilarity(dim=0)(probs[:, 1][labels == 2], translated[:, 1][labels == 2])
        sim_along_arteries = torch.nn.CosineSimilarity(dim=0)(probs[:, 2][labels == 3], translated[:, 2][labels == 3])
